...
def ranking_similarity(ont_A, ont_B, syns_A, syns_B):
...
def convert_to_synonyms(ont_A, ont_B):
    classes_A = list(ont_A.classes())
    classes_B = list(ont_B.classes())
    syns_A = []
    syns_B = []
...
def compute_string_similarity(ont_A, ont_B, hamming_threshold=1, jaro_threshold=0.95, waterman_threshold=5):
...
def main():
    path1 = "file://animalsA.owl"
    path2 = "file://animalsB.owl"
    ont_A, ont_B = load_ontologies(path1, path2)

    match_indices, hamming_indices, jaro_indices, waterman_indices = compute_string_similarity(ont_A, ont_B)

    syns_A, syns_B = convert_to_synonyms(ont_A, ont_B)
    syn_match_indices, syn_hamming_indices, syn_jaro_indices, syn_waterman_indices = compute_synonym_similarity(syns_A, syns_B)

    match_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), match_indices)
    syn_match_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), syn_match_indices)

    hamming_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), hamming_indices)
    syn_hamming_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), syn_hamming_indices)

    jaro_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), jaro_indices)
    syn_jaro_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), syn_jaro_indices)

    waterman_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), waterman_indices)
    syn_waterman_triples = create_triples(list(ont_A.classes()), list(ont_B.classes()), syn_waterman_indices)

    hamming_ranking, jaro_ranking, waterman_ranking, hamming_syns_ranking, jaro_syns_ranking, waterman_syns_ranking = ranking_similarity(ont_A, ont_B, syns_A, syns_B)
